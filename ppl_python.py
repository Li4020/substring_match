...
directories = ["python", "python_poly", "python_line"]

# ディレクトリごとの近似手法
fit_methods = {
    "python": "exponential",  # 指数近似
    "python_poly": "polynomial",   # 2次近似
    "python_line": "polynomial"  # 1次近似
}

# 近似の次数（多項式近似の場合のみ使用）
approx_degrees = {
    "python": None,  # 指数近似は次数不要
    "python_poly": 3,     # 2次近似
    "python_line": 2    # 1次近似
}

# 色とラベルを設定
colors = {"python": "red", "python_poly": "blue", "python_line": "green"}
labels = {"python": "(a*)*(?>.*b.*)", "python_poly": "a*(?>.*b.*)", "python_line": "(?>.*a.*)(?>.*b.*)"}
