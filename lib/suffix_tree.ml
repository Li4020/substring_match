...
*)
  ...
  then node, sr
    ...
    reduce_right st (sr - node_len) node.parent 
...
(* test whether anoot node  node is the root node *)
...
(*	mutable new_maximal : node LSet.t; *)
    ...
    assert (pred_opt = None || Subseq.is_empty implicit);
...
(* filtering and synthesizing, no inheritance *)
  ...
  fold_while
    ...
    module SetShiftNode = Set.Make (struct type t = (* int * int * *) node let compare = Stdlib.compare end)
